...
def test_build_skill_document_text_contains_key_sections():
    row = {
        "skill_id": "s1",
...
def test_build_and_persist_indexes_writes_all_artifacts(tmp_path: Path):
    df = pd.DataFrame(
        [
            {
                "skill_id": "s0",
    ...
    # Search: query using first doc embedding to ensure top hit is itself
    # Load FAISS index directly to grab vectors is not possible; instead just run
    # a query built from the stub embedder using same doc text generation.
    # We approximate by embedding a doc-like string.
    q = _StubEmbedder(dim=4).embed_documents(["skill_id: s0\nskill_name: A\nskill_text:\nAlpha text"], task_type="RETRIEVAL_QUERY")
